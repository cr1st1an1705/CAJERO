...
docker exec mysql-secondary-cajero mysqldump -uroot -proot --single-transaction --set-gtid-purged=OFF --databases cajero_atm > /tmp/cajero_atm_failback.sql

echo "3. Restaurando copia en primary..."
docker exec -i mysql-primary-cajero mysql -uroot -proot < /tmp/cajero_atm_failback.sql
