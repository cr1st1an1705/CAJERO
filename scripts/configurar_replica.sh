...
docker exec mysql-primary-cajero mysql -uroot -proot -e "
...
docker exec mysql-secondary-cajero mysql -uroot -proot -e "
STOP REPLICA;
RESET REPLICA ALL;
...
docker exec mysql-secondary-cajero mysql -uroot -proot -e "SHOW REPLICA STATUS\G" | grep -E "Replica_IO_Running|Replica_SQL_Running|Last_IO_Error|Last_SQL_Error"
